...
# Dev dependencies
npm install -D typescript tsx prisma @types/node @types/bcryptjs @types/uuid eslint
```
...
- outDir: dist
- rootDir: src
- paths: `@/*` → `./src/*`, `@modules/*` → `./src/modules/*`, `@libs/*` → `./src/libs/*`, `@config/*` → `./src/config/*`
...
#### `.gitignore`
Include: node_modules, dist, .env, .env.local, .env*.local, .next, *.log, .prisma
...
- Check if error is Zod validation error → 400 with validation details
...
- Use the EXACT response format from `response-handling.md`
- Use the EXACT pagination format from `pagination.md`
