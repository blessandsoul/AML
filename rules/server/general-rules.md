...
- Domain:
  ...
  - Defined by project-specific entities organized as domain modules
  - Each domain module lives under `src/modules/<domain>/`
  - Refer to the project's `prisma/schema.prisma` for the current data model
...
3. Fastify:
   ...
   - Use plugins and route registration with prefixes (e.g., /api/v1/<domain>).
...
4. Database:
   ...
   - Add proper indexes for search-heavy and filter-heavy fields.
   - Avoid N+1 queries; prefer joins or batched queries.

5. Redis:

   - Use for caching frequently accessed data (popular listings, lookups, etc.).
...
- For core business logic, provide Jest/Vitest tests.
