...
export function extractKeywordsFromFilename(media: SafeMedia): string[] {
  ...
  if (parts.length >= 2) {
    ...
    const allKeywords = `${entitySlug} ${descriptiveName}`
      ...
      .filter(word => !['tour', 'company', 'guide', 'driver', 'user', 'image', 'photo', 'logo', 'avatar'].includes(word)); // Filter generic terms
...
/**
 * Example usage for tour image gallery
 *
 * ```tsx
 * import { generateTourImageAlt } from '@/lib/utils/seo';
 *
 * export const TourGallery = ({ tour, images }) => {
 *   return (
 *     <div>
