...
export function GradientUnderline(props: SharedHeaderProps) {
                    ...
                    <nav className="hidden md:flex items-center gap-1 absolute left-1/2 -translate-x-1/2">
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                <Link
                                    ...
                                    href={item.href}
                                    className={cn(
                                        "relative px-5 py-2 text-sm font-semibold uppercase tracking-wider transition-colors duration-300",
                                        "after:absolute after:bottom-0 after:left-2 after:right-2 after:h-[2px] after:bg-gradient-to-r after:from-primary after:to-secondary after:rounded-full after:transition-transform after:duration-300 after:origin-left",
                                        isActive
                                            ? (useWhiteText ? "text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]" : "text-primary")
                                            : (useWhiteText
                                                ? "text-white/80 drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)] hover:text-white after:scale-x-0 hover:after:scale-x-100"
                                ...
                                >
                                    {isActive && (
                                        <motion.div
                                            layoutId="v5-underline"
                                            className="absolute -bottom-1 left-2 right-2 h-[2px] bg-gradient-to-r from-primary to-secondary rounded-full"
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button
                                    className={cn(
                                        "relative px-5 py-2 text-sm font-semibold uppercase tracking-wider transition-colors duration-300 flex items-center gap-1 outline-none",
                                        "after:absolute after:bottom-0 after:left-2 after:right-2 after:h-[2px] after:bg-gradient-to-r after:from-primary after:to-secondary after:rounded-full after:transition-transform after:duration-300 after:origin-left",
                    ...
                    <div className="flex items-center gap-2 z-20">
                        ...
                        {props.mounted && props.isAuthenticated && props.user ? (
                                    ...
                                    <div className="p-6 bg-gradient-to-b from-muted/50 to-background border-b border-border relative">
                                        ...
                                        <div className="mt-4 p-1 bg-muted/80 rounded-lg flex relative">
                                            <motion.div
                                                ...
                                                animate={{
                                                    left: props.viewMode === 'user' ? '4px' : '50%',
                                                    width: 'calc(50% - 6px)',
                                                    x: props.viewMode === 'dealer' ? 2 : 0
                                                }}
                                                transition={{ type: "spring", stiffness: 400, damping: 30 }}
                                            />
                                            <button onClick={() => props.setViewMode('user')} className={cn("flex-1 relative z-10 flex items-center justify-center gap-2 py-2 text-sm font-bold transition-colors duration-200", props.viewMode === 'user' ? "text-foreground" : "text-muted-foreground hover:text-foreground/80")}>
                                                <UserIcon className="w-4 h-4" /> მომხმარებელი
                                            </button>
                                            <button onClick={() => props.setViewMode('dealer')} className={cn("flex-1 relative z-10 flex items-center justify-center gap-2 py-2 text-sm font-bold transition-colors duration-200", props.viewMode === 'dealer' ? "text-foreground" : "text-muted-foreground hover:text-foreground/80")}>
                            ...
                            <div className="hidden sm:flex items-center gap-2">
                                <Button variant="ghost" size="sm" asChild className={cn(useWhiteText ? "text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)] hover:text-white hover:bg-white/10" : "text-muted-foreground hover:text-foreground")}>
                                    <Link href="/login"><LogIn className="w-4 h-4 mr-2" />შესვლა</Link>
                                </Button>
                                <Button size="sm" asChild className={cn(useWhiteText && "bg-white text-[#1C2331] hover:bg-white/90 shadow-lg")}>
