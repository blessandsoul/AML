...
import { ArrowLeft, Globe, Phone, Mail, CheckCircle, Calendar, Building2, Share2, MessageCircle } from 'lucide-react';
...
const CompanyDetailsContent = () => {
    ...
    const handleTabChange = (tab: string) => {
        ...
        params.set('tab', tab);
        // Reset tours page when switching tabs
        if (tab !== 'tours') {
    ...
    const handleToursPageChange = (page: number) => {
        const params = new URLSearchParams(searchParams);
    ...
    if (error || !company) {
        return (
            <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 text-center">
                ...
                <Button variant="outline" onClick={() => navigate('/explore/companies')}>
                    Back to Companies
                </Button>
            </div>
        );
    ...
    // Using a static premium tour placeholder for now as requested
    const coverImage = "https://images.unsplash.com/photo-1565008447742-97f6f38c985c?w=1920&q=80";
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                            ...
                            <TabsContent value="about" className="mt-8 animate-fade-in space-y-8">
                                <div className="prose prose-stone dark:prose-invert max-w-none">
                                    <h3 className="text-xl font-semibold mb-4">About {company.companyName}</h3>
                                    {company.description ? (
                                ...
                                {/* Stats or other blocks can go here */}
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                    {[
                                        { label: "Tours", value: toursData?.pagination.totalItems?.toString() || "0" },
                                        { label: "Reviews", value: "0" },
                                        { label: "Rating", value: "N/A" },
                                        { label: "Response", value: "100%" },
                                    ].map((stat, i) => (
                                        <div key={i} className="bg-card border rounded-xl p-4 text-center hover:border-primary/50 transition-colors cursor-default">
                            ...
                            <CardContent className="space-y-5 pt-6">
                                ...
                                {company.phoneNumber && (
                                    <div className="flex items-start gap-4 group">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-medium text-muted-foreground mb-0.5">Phone</p>
                                            <a href={`tel:${company.phoneNumber}`} className="text-sm font-medium hover:text-primary transition-colors">
                                                {company.phoneNumber}
                                            </a>
                                        </div>
                                    </div>
                                ...
                                {company.user?.email && (
                                    <div className="flex items-start gap-4 group">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <a href={`mailto:${company.user.email}`} className="text-sm font-medium hover:text-primary transition-colors break-all">
                                                {company.user.email}
                                            </a>
                                        </div>
                                    </div>
                        ...
                        <Card className="bg-primary/5 border-primary/10">
                            <CardContent className="p-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-foreground">Verified Partner</h4>
                                        <p className="text-xs text-muted-foreground mt-1">This company has been verified by Atlas Caucasus for quality and reliability.</p>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
