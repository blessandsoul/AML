    ...
    return (
        <div className={cn("space-y-8", className)}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                {tour.maxPeople && (
                    <div className="flex flex-col gap-1 p-4 rounded-xl bg-card border shadow-sm">
                        ...
                        <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Group Size</span>
                        <span className="font-medium text-foreground">Up to {tour.maxPeople}</span>
                    </div>
                )}
                ...
                {tour.difficulty && (
                    <div className="flex flex-col gap-1 p-4 rounded-xl bg-card border shadow-sm">
                        ...
                        <span className="font-medium text-foreground capitalize">{tour.difficulty}</span>
                    </div>
                )}
                ...
                {tour.startLocation && (
                    <div className="flex flex-col gap-1 p-4 rounded-xl bg-card border shadow-sm">
                        ...
                        <span className="font-medium text-foreground truncate" title={tour.startLocation}>
                            {tour.startLocation}
                        </span>
                    </div>
                )}
            ...
            <div className="space-y-4">
                <h2 className="text-2xl font-bold tracking-tight">About this Experience</h2>
                <div className="prose prose-slate dark:prose-invert max-w-none text-muted-foreground leading-relaxed">
                    {/* Note: In a real app we might need to sanitize HTML if description is rich text */}
                    <p className="whitespace-pre-line">{tour.description || tour.summary || "No description available."}</p>
                </div>
            </div>
        </div>
    );
