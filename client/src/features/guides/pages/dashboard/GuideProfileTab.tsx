...
import { useMyGuide, useUpdateGuide, useDeleteGuide, useUploadGuidePhotos, useDeleteGuidePhoto } from '../../hooks/useGuide';
import { getErrorMessage } from '@/lib/utils/error';
import { getMediaUrl } from '@/lib/utils/media';

// Form schema (no transformation here to keep types simple)
...
// Manual type definition to handle form state flexible types (string from input vs number from schema)
                    ...
                    {isEditing ? (
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <Label htmlFor="yearsOfExperience">{t('guide.experience', 'Years of Experience')}</Label>
                                    <Input
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="languages">{t('guide.languages', 'Languages (comma separated)')}</Label>
                                    <Input
                                        id="languages"
                                        {...register('languages')}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                                <div className="space-y-1">
                                    ...
                                    <p className="font-medium">{guide.yearsOfExperience} {t('common.years', 'years')}</p>
                                </div>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex flex-wrap gap-2 pt-1">
                                        {(Array.isArray(guide.languages) ? guide.languages : (typeof guide.languages === 'string' ? (guide.languages as string).split(',') : []))?.map((lang) => (
                                            <span key={lang} className="inline-flex items-center rounded-md border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-secondary text-secondary-foreground hover:bg-secondary/80">
                                                {lang.trim()}
                                            </span>
                                        )) || '-'}
                                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border border-destructive/30 rounded-lg bg-destructive/5">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {t('guide.delete_desc', 'Once deleted, your guide profile and all associated data will be removed.')}
                            </p>
                        </div>
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    <AlertDialogTitle>
                                        {t('guide.delete_confirm_title', 'Are you sure?')}
                                    </AlertDialogTitle>
                                    <AlertDialogDescription>
                                        {t('guide.delete_confirm_desc', 'This action cannot be undone. Your guide profile will be permanently removed.')}
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
