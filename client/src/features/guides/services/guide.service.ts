...
class GuideService {
    async getGuides(params: GuideFilters = {}): Promise<GuidePaginatedResponse> {
        const response = await apiClient.get(API_ENDPOINTS.GUIDES.LIST, { params });
        return response.data.data;
    }

    async getGuide(id: string): Promise<Guide> {
        const response = await apiClient.get(API_ENDPOINTS.GUIDES.GET(id));
        return response.data.data;
    }

    async getMyGuide(): Promise<Guide> {
        const response = await apiClient.get(API_ENDPOINTS.GUIDES.MY);
        return response.data.data;
    }

    async updateGuide(id: string, data: Partial<Guide>): Promise<Guide> {
        const response = await apiClient.patch(API_ENDPOINTS.GUIDES.UPDATE(id), data);
        return response.data.data;
    }
