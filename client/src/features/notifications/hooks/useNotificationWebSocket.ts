...
export const useNotificationWebSocket = () => {
    ...
    useEffect(() => {
        console.log('[NotificationWebSocket] Setting up subscription, status:', status);

        // Subscribe to notification events
        const unsubscribe = subscribe(MessageType.NOTIFICATION, (wsMessage) => {
            console.log('[NotificationWebSocket] Received notification:', wsMessage);
            const payload = wsMessage.payload as NotificationPayload;
            const notification = payload.notification;

            console.log('[NotificationWebSocket] Notification data:', notification);
            ...
            queryClient.setQueryData<IUnreadCountResponse>(
                ...
                (oldData) => {
                    ...
                    console.log('[NotificationWebSocket] Updating unread count:', oldData?.count, '->', newCount);
