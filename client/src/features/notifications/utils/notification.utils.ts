...
export const groupNotifications = (notifications: INotification[]): IGroupedNotification[] => {
    ...
    notifications.forEach((notification) => {
        ...
        if (shouldGroup && groups[groupKey]) {
            ...
            // If the current one is unread, ensure the group is marked unread (if it wasn't already for some reason)
            // Though usually if top one is unread, group is unread.
            // If top one is read but this one is unread, we should probably mark group as unread?
            // But 'isRead' on group head usually dictates UI state.
            // Let's assume if ANY in group is unread, the group is unread.
