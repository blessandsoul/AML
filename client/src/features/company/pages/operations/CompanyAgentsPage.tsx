import { useQuery } from '@tanstack/react-query';
import { useTranslation } from 'react-i18next';
import { Users, Mail, Calendar } from 'lucide-react';
import { companyService } from '../../services/company.service';
import { formatDate } from '@/lib/utils/format';
import { Badge } from '@/components/ui/badge';
import { DataTable } from '@/components/common/DataTable';
import type { ColumnDef } from '@/components/common/DataTable';
import { ErrorMessage } from '@/components/common/ErrorMessage';
import type { TourAgent } from '../../types/company.types';


export const CompanyAgentsPage = () => {
    const { t } = useTranslation();

    const { data: response, isLoading, error } = useQuery({
        queryKey: ['company', 'agents'],
        queryFn: () => companyService.getTourAgents(),
    });

    const agents = response?.tourAgents || [];

    const columns: ColumnDef<TourAgent>[] = [
        {
            header: t('company.agents.table.agent'),
            cell: (agent) => (
                <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">
                        {agent.firstName[0]}
                    </div>
                    <div>
                        <div className="font-semibold">{agent.firstName} {agent.lastName}</div>
                        <div className="text-xs text-muted-foreground hidden sm:block">ID: {agent.id.slice(0, 8)}...</div>
                    </div>
                </div>
            ),
            className: "w-[250px] font-medium"
        },
        {
            header: t('company.agents.table.contact'),
            cell: (agent) => (
                <div className="flex flex-col gap-1 text-sm">
                    <div className="flex items-center gap-2 text-muted-foreground">
                        <Mail className="h-3.5 w-3.5" />
                        {agent.email}
                    </div>
                </div>
            )
        },
        {
            header: t('company.agents.table.status'),
            cell: (agent) => (
                <Badge variant={agent.emailVerified ? "outline" : "secondary"} className={agent.emailVerified ? "text-green-600 border-green-200 bg-green-50" : "text-amber-600 bg-amber-50"}>
                    {agent.emailVerified ? t('common.verified') : t('common.pending')}
                </Badge>
            )
        },
        {
            header: t('company.agents.table.joined'),
            cell: (agent) => (
                <div className="flex items-center gap-2 text-muted-foreground">
                    <Calendar className="h-3.5 w-3.5" />
                    {formatDate(agent.createdAt)}
                </div>
            )
        }
    ];

    if (error) {
        return <ErrorMessage error={error} />;
    }

    return (
        <div className="space-y-6">
            <DataTable
                data={agents}
                columns={columns}
                isLoading={isLoading}
                emptyState={{
                    icon: Users,
                    title: t('company.agents.empty_state'),
                    description: t('company.agents.empty_state_description', 'No agents found for your company.')
                }}
            />
        </div>
    );
};

