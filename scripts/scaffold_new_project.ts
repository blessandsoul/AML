...
// --- CONFIGURATION ---
const ROOT_DIR = path.resolve(__dirname, '..'); // Assuming script is in /scripts
...
const SERVER_MODULES_TO_REMOVE = [
    ...
    'inquiries',
    'reviews',
    // Keep: auth, users, companies, locations, media, chat, notifications, admin
];
...
const CLIENT_FEATURES_TO_REMOVE = [
    ...
    'drivers',
    'inquiries',
    // Keep: auth, users, companies, chat, notifications, dashboard, admin
];
...
enum UserRole {
  ...
  COMPANY
  ADMIN
  // Add other roles as needed (e.g. DEALER, AGENT)
}
...
enum NotificationType {
  ...
  CHAT_MENTION
  SYSTEM
  // Add specific notification types here
}
...
// ==========================================
// COMPANY PROFILE (Replace strict fields with JSON if needed)
// ==========================================
...
model Company {
  ...
  // Custom Metadata for specific business logic
...
function copyRecursiveSync(src: string, dest: string, isRoot = false) {
    const stats = fs.statSync(src);
    if (stats.isDirectory()) {
        if (isRoot) {
            // Just checking root level ignores
        }

        const dirName = path.basename(src);
        if (!isRoot && IGNORE_DIRS.includes(dirName)) return;

        // Check specific module removals
        if (src.includes('server/src/modules') && SERVER_MODULES_TO_REMOVE.includes(dirName)) return;
        if (src.includes('client/src/features') && CLIENT_FEATURES_TO_REMOVE.includes(dirName)) return;
...
function processServerAppTs() {
    ...
    // Remove imports
    content = content.replace(/import { .*Routes } from "__modules/tours /.* ";\n?/g, '')
        .replace(/import { .*Routes } from "__modules/guides /.* ";\n?/g, '')
            .replace(/import { .*Routes } from "__modules/drivers /.* ";\n?/g, '')
                .replace(/import { .*Routes } from "__modules/inquiries /.* ";\n?/g, '')
                    .replace(/import { .*Routes } from "__modules/reviews /.* ";\n?/g, '');

  // Use generic regex for imports just in case
  content = content.replace(/import { (tour|guide|driver|inquiry|review)Routes } from ".\/modules\/.*";\n?/g, '');

    // Remove registrations
    content = content.replace(/app.register\((tour|guide|driver|inquiry|review)Routes, { prefix: "\/api\/v1" }\);\n?/g, '');
...
function processClientRouter() {
    ...
    // Naive cleaning - In a real scenario we'd use AST, but for this scaffold, simple removal of imports is enough.
    // The routes themselves will cause errors if components are missing, so we must remove the route definitions too.
    // For simplicity in this script, we will just comment out the "routes" array content or replace it with a minimal version.
    // OR BETTER: Just fail gracefully if we can't parse.
    // Strategy: Replace imports with empty string

    // Remove imports
    content = content.replace(/import { .* } from '@\/features\/(tours|guides|drivers|inquiries).*';\n?/g, '');

    // We can't easily remove the route objects with regex safely. 
    // Instead, let's create a MINIMAL router.tsx for the scaffold.
    const minimalRouter = `import { createBrowserRouter } from 'react-router-dom';
...
export const router = createBrowserRouter([
    {
        path: '/',
        element: <MainLayout />,
        errorElement: <ErrorBoundary><div>404</div></ErrorBoundary>,
        children: [
...
function processClientHeader() {
    const headerPath = path.join(DEST_DIR, 'client', 'src', 'components', 'layout', 'Header.tsx');
    // Replace logic inside header is complex. Let's just strip the "NavMenu" items related to tours.
    // Or better, let's just leave the Core generic "Home" link.
    // User can rebuild navigation.

    // For now, I will leave the header as is but maybe comment out the specific nav items? 
    // No, that will break build if I removed the routes constant. 
    // I need to update ROUTES constant too? NO, ROUTES constant is in `lib/constants`.

    // Let's replace Header with a simplified version.

    // Reading original imports to keep them
    // Actually, writing a simplified header is safer than regex replacing React components.
}
