    ...
    for (const media of allMedia) {
      ...
      if (!entityExists) {
        ...
        try {
          ...
          logger.error(`[MediaCleanup] Failed to delete media ${media.id}:`, err);
        }
      }
    }
  } catch (err) {
    logger.error("[MediaCleanup] Error in cleanupOrphanedRecords:", err);
    throw err;
  }
      ...
      for (const filename of filesOnDisk) {
        ...
        if (!mediaRecord) {
          ...
          try {
            ...
            logger.error(`[MediaCleanup] Failed to delete file ${url}:`, err);
          }
        }
      }
    }
  } catch (err) {
    logger.error("[MediaCleanup] Error in cleanupOrphanedFiles:", err);
    throw err;
  }
